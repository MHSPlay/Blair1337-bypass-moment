...
bool ImGui::Checkbox( const char* label, bool* v )
{
    ...
    const float check_bb_width = square_sz + style.ItemInnerSpacing.x + label_size.x;
    const ImVec2 pos = ImVec2( window->WorkRect.Max.x - 25, window->DC.CursorPos.y ); // Align checkbox to the right
    const ImRect total_bb( pos, pos + ImVec2( check_bb_width, label_size.y + style.FramePadding.y * 2.0f ) );
    ItemSize( total_bb, style.FramePadding.y );
    ...
    RenderFrame( check_bb.Min, check_bb.Max, ImColor( 291, 0, 255 ), true, 6.0f ); // Rounded corners of 6
    ImU32 check_col = GetColorU32( ImGuiCol_CheckMark );
    if ( *v )
    {
        const float pad = 6.0f;
        RenderCheckMark( window->DrawList, ImVec2( check_bb.Min.x + pad, check_bb.Min.y + pad ), ImColor( 255, 255, 255 ), square_sz - pad * 2.0f ); // White checkmark
    }

    ImVec2 label_pos = ImVec2( ImGui::GetWindowPos( ).x + 5, check_bb.Min.y + 5 );
    if ( g.LogEnabled )
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags )
{
    ...
    const float w = CalcItemWidth( );
    const ImVec2 label_size = CalcTextSize( label, NULL, true );
    const float scaleFactor = 0.61f; // Adjust this scale factor to make it smaller
    float scale_no_dpi = 300.f;
    const ImRect frame_bb( window->DC.CursorPos + ImVec2( scale_no_dpi, 0 ), window->DC.CursorPos + ImVec2( scale_no_dpi, 0 ) + ImVec2( w * scaleFactor, label_size.y * scaleFactor + style.FramePadding.y * 2.0f * scaleFactor ) );
    const ImRect total_bb( frame_bb.Min, frame_bb.Max + ImVec2( label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x * scaleFactor : 0.0f, 0.0f ) );
    ...
    // Slider behavior
    ImRect grab_bb;
    float value = *( float* ) p_data;// Slider behavior
    float sliderPercentage = ( value - *( float* ) p_min ) / ( *( float* ) p_max - *( float* ) p_min );
    const float sliderEndX = window->Pos.x + window->Size.x - 5; // End of X of window minus 5
    const float grabWidth = ImMin( sliderEndX - frame_bb.Min.x, ( value / ( *( float* ) p_max - *( float* ) p_min ) ) * frame_bb.GetWidth( ) );


    ImVec2 grabMin = ImVec2( frame_bb.Min.x, frame_bb.Min.y );
    ImVec2 grabMax = ImVec2( frame_bb.Min.x + grabWidth, frame_bb.Max.y );
    ImVec2 sliderFillMin = ImVec2( frame_bb.Min.x, frame_bb.Min.y );
    ImVec2 sliderFillMax = ImVec2( frame_bb.Min.x + grabWidth, frame_bb.Max.y );
    window->DrawList->AddRectFilled( sliderFillMin, sliderFillMax, GetColorU32( ( g.ActiveId == id ) ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab ), style.GrabRounding );
    grab_bb = ImRect( grabMin, grabMax );
    const bool value_changed = SliderBehavior( frame_bb, id, data_type, &value, p_min, p_max, format, flags, &grab_bb );
    ...
    auto full = ( std::string( label ) + " (" + value_buf + ")" );
    if ( label_size.x > 0.0f )
        RenderText( ImVec2( window->Pos.x + 5, frame_bb.Min.y ), full.c_str( ) );
